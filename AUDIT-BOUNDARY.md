...
|----------|--------------|---------------------|
| **VBW novel IP** (Rust, `src/`) | ~1,300 | Full line-by-line review |
...
**Total novel code requiring audit: ~1,300 lines of Rust.**
...
#### `src/fs_guard.rs` (23 lines) -- Defensive File I/O

**What it does:**
- Single source of truth for symlink-checked, size-bounded file reads
- Used by `main.rs` and `policy.rs` for all untrusted file loading
- Documents the TOCTOU risk acceptance inline

**What an auditor should focus on:**
- This is the one place to audit filesystem guard logic. All callers delegate
  here instead of implementing their own checks. See **Known Limitations** for
  the accepted TOCTOU window.

#### `src/main.rs` (576 lines) -- CLI Orchestration
...
- Delegates defensive file reading to `fs_guard::read_validated`
...
#### `src/independence.rs` (227 lines) -- Independence Enforcement Engine
...
#### `src/attest.rs` (110 lines) -- Attestation Generation
...
#### `src/policy.rs` (156 lines) -- Policy Configuration

**What it does:**
- Defines the `VbwPolicy` struct (5 boolean/list fields)
- Loads policy from JSON file via `fs_guard::read_validated`
...
Trivial. Exports the five modules above.
...
**Location:** `src/fs_guard.rs` (single source of truth; called by `main.rs`
and `policy.rs`)
...
change localized to `fs_guard::read_validated()`.
